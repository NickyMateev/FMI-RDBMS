...
CREATE FUNCTION FN71637.RETRIEVE_SCHEDULE_FOR_DATE(V_FLIGHT_DATE DATE)
...
/* Demonstration:
SELECT *
  FROM TABLE(FN71637.RETRIEVE_SCHEDULE_FOR_DATE('2012-05-23'));
*/

CREATE FUNCTION FN71637.CALC_PASSENGERS_FOR_DATE_PERIOD(V_AIRLINE_CODE CHAR(2), V_START_DATE DATE, V_END_DATE DATE)
  RETURNS INTEGER
  RETURN
  SELECT COUNT(*)
...
/* Demonstration:
SELECT FN71637.CALC_PASSENGERS_FOR_DATE_PERIOD('AA', '1990-01-01', CURRENT_DATE)
FROM FLIGHTS
FETCH FIRST ROW ONLY;
*/

CREATE FUNCTION FN71637.GET_LAST_MONTH_DATE()
RETURNS DATE
RETURN
...
CREATE VIEW FN71637.FLIGHT_SUMMARY_VIEW
  AS
SELECT A.NAME AS "AIRLINE NAME", DAILY_SCHEDULE.FLIGHT_NUMBER, DAILY_SCHEDULE.FLIGHT_DATE,
  ...
  FN71637.CALC_PASSENGERS_FOR_DATE_PERIOD('AA', FN71637.GET_LAST_MONTH_DATE(), FN71637.GET_LAST_MONTH_DATE() + 1 MONTH - 1 DAY) AS "PREVIOUS MONTH PASSENGER COUNT"
FROM TABLE(FN71637.RETRIEVE_SCHEDULE_FOR_DATE(CURRENT_DATE)) AS DAILY_SCHEDULE, FLIGHTS AS F, AIRLINES AS A
WHERE DAILY_SCHEDULE.FLIGHT_NUMBER = F.FLIGHT_NUMBER
  AND F.AIRLINE_CODE = A.CODE;

/* Demontration:
  SELECT *
  FROM FN71637.FLIGHT_SUMMARY_VIEW;
*/
