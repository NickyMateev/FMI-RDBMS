...
SELECT LASTNAME, SALARY, DECIMAL(SALARY * 1.05, 10, 2) AS "INC-Y-SALARY", DECIMAL((SALARY * 1.05) / 12, 10, 2) AS "INC-M-SALARY"
FROM EMPLOYEE
...
SELECT LASTNAME, EDLEVEL, SALARY + 1200 AS "NEW-SALARY", DECIMAL(BONUS * 0.5, 10, 2) AS "NEW-BONUS"
FROM EMPLOYEE
WHERE EDLEVEL IN (18, 20)
ORDER BY EDLEVEL DESC, "NEW-SALARY";
...
SELECT PROJNO, DECIMAL(DAYS(PRENDATE) - DAYS(PRSTDATE) / 7.0 + 0.05, 10, 1) AS WEEKS
FROM PROJECT
...
SELECT PROJNO, PRENDATE AS ESTIMATED, PRENDATE + (DAYS(PRENDATE) - DAYS(PRSTDATE) * 1.1) DAYS AS EXPECTED  
FROM PROJECT
WHERE PROJNO LIKE 'MA%'
ORDER BY PROJNO;
